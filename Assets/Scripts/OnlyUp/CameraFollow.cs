...
namespace OnlyUp
{
    ...
    public class CameraFollow : MonoBehaviour
    {
        ...
        private void Update()
        {
            // 일시정지 메뉴가 열려있는 동안은 커서 잠금/카메라 회전을 전혀 건드리지 않는다.
            // Update()는 Time.timeScale이 0이어도 계속 호출되므로, 이 가드가 없으면 메뉴 버튼을
            // 클릭한 바로 그 좌클릭을 여기서도 감지해 커서를 다시 잠그고 숨겨버려서(같은 프레임에
            // 클릭 좌표가 화면 중앙으로 튀어) 버튼이 눌러도 반응 없는 것처럼 보이는 문제가 있었다.
            if (OnlyUp.PauseMenuUI.IsPaused) return;
